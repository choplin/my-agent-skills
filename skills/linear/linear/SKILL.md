...
Within each block, group the Issues by `state` type and count every Issue in each group:
...
These three are the "what's in flight" axis and are always shown. Optionally append **Done** (`completed`) as a single total in parentheses to convey progress for Project blocks; **exclude `canceled`** from the counts. Do not query or show completed Issues merely to decorate the No Project block.
