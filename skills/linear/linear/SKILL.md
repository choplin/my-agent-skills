...
| **Issue** | The unit of work. **1 Issue = 1 PR = 1 branch**, and **self-complete** (see below). | Keeps PRs reviewable and lets a context-free executor pick up any Todo. |
...
| **Label** | Two single-select groups: **Type** and **Repo** (see Label groups). | Cheap, queryable classification; single-select keeps each axis unambiguous. |
...
- **Acceptance** — how "done" is judged: expected behavior, and the check to run (test/command) when one exists.
...
Grooming is where a rough Backlog item becomes a ready Todo. Two things are settled here, together:

1. **Self-completeness** — bring the description up to the authoring standard above.
2. **True size** — decide whether it's really one PR, or splits into several.
...
- **Type** (one per issue): `feat` / `fix` / `refactor` / `build` / `docs` / `test` / `chore` — matching the conventional-commit types used in commits. The issue's primary change kind.
...
### Initial setup (this workspace starts from Linear defaults)

The workspace currently has ungrouped default labels (`Bug`, `Feature`, `Improvement`) and no groups. To reach the convention (create groups with `isGroup: true`, then create members with `parent: <group>` — both supported by the label-create MCP tool):

1. Create the **Type** group and its 7 members (`feat`…`chore`). Retire the old `Bug`/`Feature`/`Improvement` (or fold `Bug→fix`, `Feature→feat`, `Improvement→refactor`).
