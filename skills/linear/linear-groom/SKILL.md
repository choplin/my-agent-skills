---
name: linear-groom
description: Groom the Backlog of the current repository's Linear Project — resolve the repo to its active Linear Project, auto-pick Backlog issues one at a time in priority order, and walk each through grooming (Backlog → Todo) interactively in this session until no groomable Backlog item remains or the user stops. The counterpart to linear-start: linear-start auto-picks Todo work to execute; this auto-picks Backlog work to groom. Use when the user wants to work through the Backlog without naming issues one by one. Triggers on "groom the backlog", "start grooming", "backlog を整理", "Backlog を groom して", "次の backlog を groom". Should NOT trigger for starting/executing a groomed issue (use linear-start), one-off issue creation or grooming a single named issue (use linear), Jira (jira-cli), or GitHub Issues (github tools).
user-invocable: true
---

# Groom the Backlog of a Linear Project

Work through the **current repository's** Backlog, one issue at a time, turning rough Backlog items into ready **Todo** work orders. This skill covers *selection and looping*; the grooming rules themselves are owned by the `linear` skill — read its **Issue authoring standard**, **Grouping & ordering**, and **The grooming step** before starting if unfamiliar.
...
Resolve the repo to its active Linear Project **exactly as `linear-start` Steps 1–2**: derive the Repo label from the git repo, then select the single active Project tagged with `Repo/R`. The same edge cases apply (no/ambiguous Repo label → ask; missing `Repo/R` project-label → ask the user to create it in the UI; multiple active Projects → ask; none → stop and offer to create one).
...
2. **Groom it interactively**, following `linear`'s grooming step. Grooming is **not** automated — do it *with* the user in this session, because What/Why/Acceptance often live only in the user's head. Settle the three things together:
   - **Self-completeness** — bring the description up to `linear`'s authoring standard (What & why / Where / Acceptance / Constraints). Ask the user for whatever is missing; propose a draft description and confirm.
   - **True size** — decide whether it's really one deliverable or splits. If it splits, apply `linear`'s grouping rule and its promote-vs-rebuild asymmetry (small multi-PR → promote to parent + sub-issues; distinct outcome → build a Project, Cancel+link or reuse the original). Add `blocked by` relations for execution order when needed.
