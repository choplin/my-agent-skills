...
- **A running-example map — only if the paper leans on one worked/running example that more than one perspective will discuss.** Build it once, here — but note the trap: once handed down, this map is the *single source* for every report that uses it, so an error in it propagates to all of them and no cross-report check can catch it (the reports will agree, on the wrong thing). Single-sourcing removes the divergence that would otherwise flag a mistake, so the map must be right at construction, not merely consistent. Build it against ground truth, not prose:
  ...
  - **Before handing the map down, re-open the figure and confirm each row's attribution actually sits on the element you cited.** This is the teeth: the map is not done until every row is figure-verified.
  This is then handed to every perspective that touches the example (like the thesis) so each references one *verified* attribution instead of re-deriving it and diverging. Skip the whole map for papers without such an example.
...
6. **Running-example map correction — if any perspective flagged a map/figure discrepancy in its reply.** Re-open the cited figure image, decide the correct attribution from the figure (it wins over OCR prose), fix that row in the map, and fix every report that inherited the wrong row (grep the reports for the mis-attributed phrasing). Because the map is single-sourced, one wrong row is consistently wrong across reports — correct it in all of them, not just the one that flagged it.
7. **Print a manifest**: the work dir path (the renamed slug dir), each report, and `ocr/` contents (OCR always runs via MinerU). Note any figures recovered or dropped in step 1.
...
- [ ] If the paper has a running example that more than one report discusses, the Phase 1 running-example map was built by opening the original figure image (each row anchored to a named figure element in its evidence column, the figure winning over any conflicting OCR prose), and those reports attribute the example's features per that verified map — so the single shared source is correct, not merely consistent. (Single-sourcing hides a wrong attribution from cross-report checks, so the map's own figure-verification is the check that matters.)
