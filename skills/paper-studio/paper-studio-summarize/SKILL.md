...
write spine.md (confirmed facts)            reports/experiments.md               (paper-studio-consistency-
...
7. **Point to the next steps** (do not run them unprompted): the reports can now become a two-speaker **audio guide** of the overview (`pdf-studio-audio-dialogue` on `reports/overview.md` → `pdf-studio-audio-narrate`) and a browsable **website** (`paper-studio-generate-site`, the paper-tuned site builder — orders pages by perspective and labels them 背景 / 手法 / …, then `pdf-studio-deploy-site` to publish). To run the whole chain (summary → overview audio → site) in one shot, use `paper-studio-full-guide`.
