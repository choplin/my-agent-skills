...
- **`pdfinfo` / `pdftotext` need system `poppler`.** `pdfinfo` does the page-count pre-check; `pdftotext` supplies the text layer that `mineru_ocr.sh` refills the body text from. If either is missing, install poppler (macOS: `brew install poppler`; Debian/Ubuntu: `apt-get install poppler-utils`).
