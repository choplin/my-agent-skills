...
- **MinerU is a hard requirement — there is no fallback.** This skill reads the paper through local MinerU (`mineru_ocr.sh`), which runs entirely on the machine (nothing is uploaded; safe for confidential manuscripts). MinerU is resolved automatically — an already-installed `mineru` on PATH is used as-is; otherwise it is provisioned in-repo via `uv run` (declared in `pyproject.toml`), so there is no manual install — but its first run downloads model weights (several GB), plus a `.venv` sync when resolved via uv, so it is slow: warn about that. If OCR errors out, report it and stop; do NOT fall back to visual reading, to an external OCR API, or to any other reader.
...
- **Do not install anything by hand.** MinerU is used from PATH if already installed, else provisioned automatically by `uv run` from `pyproject.toml`/`uv.lock` (a project-local `.venv`, synced on first use) — never run an ad-hoc `pip`, `uv tool install`, or `uv add`. If provisioning fails (e.g. no network — see the sandbox Gotcha below), report the error and stop; do not work around it.
...
2. **No install step for MinerU — it is resolved automatically.** The runtime env (poppler + a MinerU source) is resolved by the `scripts/preflight.sh` wrapper that launches `mineru_ocr.sh` (PATH, else the bundled flake); MinerU is used from PATH if installed, else synced by `uv run` from `pyproject.toml`/`uv.lock` on first use. MinerU is the skill's OCR engine and is mandatory by design (local processing keeps under-review manuscripts off the network) — there is no fallback reader. Just be ready for a **slow first run** (several-GB model download, plus a venv sync when resolved via uv); see the runtime-env Gotcha above for the one-shot `nix develop <SKILL_DIR>` setup.
...
- **A running-example map — only if the paper leans on one worked/running example that more than one perspective will discuss.** Build it once, here, reading the example off the *original figure* in `ocr/figures/` (multi-column OCR prose scrambles word order, so which element a label sits on must come from the image, not the sentence). Make a compact table: each distinct feature of the example → the concept/label the paper attributes it to → `[pNN]`. This is handed to every perspective that touches the example (like the thesis) so each references one authoritative attribution instead of re-deriving it and diverging. Skip it for papers without such an example.
...
- For any perspective that discusses the running example (typically `background` and `method`), when a running-example map was built in Phase 1: that map — so each such report reads the example's feature→concept attributions off one authoritative table instead of re-deriving them from OCR prose and diverging. This is the same hand-down discipline as the thesis, applied to the other core object multiple reports restate.
...
**Context hygiene:** each perspective writes its report to a file and returns only a path + one-line status. Do not read the finished detail reports back into the orchestrator — only two *narrow* reads at Finalize are allowed: the "次に読むべき論文" section of `related-work.md` (for overview item 6), and — for the diagram-order sweep — the fenced diagram block plus its one describing sentence in any report that contains a self-drawn diagram. Never fold a full detail report back into the overview context. (When dispatching to subagents this is automatic; when applying the skill inline, keep the same discipline.)
...
5. **Diagram-order sweep — teeth for the self-drawn-figure check.** Scan each `reports/*.md` for a fenced self-drawn diagram (```` ```mermaid ````). For every report that has one, read *only* that diagram block plus the report's own sentence(s) describing the same flow, and confirm the diagram's step order and causal direction match both the prose and the source's explicit order (e.g. prose "after assigning X, we did Y" ⇒ the diagram shows X→Y, never Y→X; a capability list is not a timeline). Have the report's author fix any diagram that contradicts its own prose. This is the narrow read-back allowed by Context hygiene — the diagram block and its describing sentence only. *Why:* the write-time self-check on diagrams has slipped through before, so the deliverable needs a check with teeth, not only a discipline.
6. **Print a manifest**: the work dir path (the renamed slug dir), each report, and `ocr/` contents (OCR always runs via MinerU). Note any figures recovered or dropped in step 1.
...
- [ ] If the paper has a running example that more than one report discusses, those reports attribute its features to the same concepts/labels — because each was handed the Phase 1 running-example map (built from the original figure) rather than re-deriving attributions from OCR prose.
- [ ] Every self-drawn diagram (`mermaid` etc.) in a report has its step order and causal direction matching that report's own prose and the source's explicit order — verified at Finalize by the diagram-order sweep, not only asserted at write time.
