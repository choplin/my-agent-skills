...
- For any perspective that discusses the paper's running/worked example (typically `background` and `method`), when the paper has one: the running-example map — a compact table of each example feature → the concept/label the paper attributes it to (`[pNN]`), built once in Phase 1 from the original figure, with an evidence column naming the figure element each row was read off. Use it as the authoritative attribution; do not re-derive attributions from the OCR prose. Safety net: if one of *your assigned* figures is open in front of you and plainly contradicts a map row, the figure wins — follow the figure and flag the discrepancy in your reply (do not silently rewrite the map's wording to match, or the reports re-diverge). The map is a single shared source, so a wrong row would otherwise propagate to every report unchecked.
...
Return only the file path and a one-line summary (do not return the body). For related-work, also state how many entries were dblp-verified vs unverified. If an assigned figure contradicted a handed-down running-example map row, say so in one line (which row, what the figure shows) so the orchestrator can correct the shared map.
