...
- For any perspective that discusses the paper's running/worked example (typically `background` and `method`), when the paper has one: the running-example map — a compact table of each example feature → the concept/label the paper attributes it to (`[pNN]`), built once in Phase 1 from the original figure. Use it as the authoritative attribution; do not re-derive attributions from the OCR prose.
