...
description: >-
  ...
  as a note, so it is not lost and can be reached later. Writes a note (default
  type `fleeting`) into the current repo's scope directory, tags it, and links it
  to related existing notes. Use when a valuable finding, decision, or fact
...
notebook if missing) and resolve the scope directory `$scope` (repo-name, or
`global`). This skill handles only the capture half.

## Steps

1. **Setup + scope** — per llm-wiki-base. Ensure `mkdir -p "$wiki/$scope"`.
...
3. **Create the note** (default type `fleeting`), piping the body via stdin:
   ```sh
   printf '%s' "<body>" |
...
4. **Tag it.** The template seeds `tags: [fleeting]`. Add topical tags by editing
   the frontmatter (e.g. `tags: [fleeting, raft, consensus]`). Keep the type tag
   as `fleeting` unless this is clearly already a distilled note — in that case
   set `permanent` / `project` and meet llm-wiki-distill's bar.
...
- [ ] Has a type tag (`fleeting` by default) plus at least one topical tag.
