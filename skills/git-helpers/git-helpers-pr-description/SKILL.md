...
description: >-
  Writes or revises the body of a non-trivial pull request so reviewers can
  follow its purpose, its design, and where each design choice lands in the
  code. Applies whenever a PR description is being written for a change large
  enough that the diff does not explain itself.
...
If the repository has a PR template (`.github/pull_request_template.md` or variants — same lookup as `git-helpers-draft-pr`):
...
- `git-helpers-draft-pr` handles **push + `gh pr create --draft`**. When the change is large (per §1 heuristics), `git-helpers-draft-pr` should call this skill to author the body before creating the PR.
