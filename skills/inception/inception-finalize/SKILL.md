...
description: >-
  ...
  the session's graph, writes it to a durable Markdown store as one keep-forever
  note tagged prd, hands the concrete actions to a tracker, and
...
The durable output is always the same shape: **one consolidated PRD note** in the selected durable Markdown store, actions handed to a tracker, everything transient discarded.

## What it produces

- **One consolidated PRD note** in the selected durable Markdown store (durable, keep-forever) — tagged `prd`, written in Japanese for its owner.
...
2. **Write the durable PRD.** Delegate to `workflow-adapter-markdown-create` with three things:
   ...
   Everything else about the note — which scope it lands in, its filename, its frontmatter, how it links and gets indexed — belongs to the adapter and selected provider. Finalize adds no storage scheme of its own.
...
- **One-way boundary.** After finalize, the durable PRD is the source of truth and the graph is archived. If the user wants to reopen the thinking, start a fresh `inception` session or edit the durable note directly — do **not** keep re-rendering the old graph, or you recreate the two-source provenance problem inception exists to kill.
