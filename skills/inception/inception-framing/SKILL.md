...
- The top of the foundational PRD set in the graph: `session.summary`, `session.background`, `session.problem`, `session.purpose`, `session.centralQuestion`, and a first cut at `session.targetUsers`. See `inception-base/references/knowledge/prd-standard.md` for what a good answer to each looks like.
