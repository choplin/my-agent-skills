...
- **Figure harvest (MinerU) is an optional enhancement with a clean fallback, not a hard dependency.** Phase 0 runs `figure_harvest.sh` to crop genuine figures (diagrams / plots / photos — not tables or console output) into `ocr/figures/`. It needs the `mineru` CLI; if `mineru` is missing the script exits non-zero and prints the install command — **relay it and continue the pipeline without figure crops** (the reports still describe figures in prose, as before). Do NOT install anything. **MinerU 3.x starts a local service and uses multiprocessing that the command sandbox blocks** (`Operation not permitted` on a semaphore) — run `figure_harvest.sh` with the sandbox disabled. It is fully local (nothing is uploaded); its first run downloads model weights (several GB) and is slow — warn about that.
...
3. Detect optional tools (do not install any): `command -v mineru` decides whether Phase 0 figure harvest runs (missing → skip it, note in the manifest). `bash <SKILL_DIR>/scripts/text_layer.sh --probe <pdf> <body-start-guess>` (any page in the body) decides whether the text-layer option can be offered.
...
Skip this phase if `mineru` is not installed (noted in Prerequisites) — the pipeline continues and figures stay described-in-prose as before. Otherwise, run once over the body range **with the sandbox disabled** (MinerU's local service + multiprocessing are blocked under the sandbox):

```
bash <SKILL_DIR>/scripts/figure_harvest.sh <pdf-abs-path> <WORK_DIR>/ocr [body-start] [body-end]
```

Pass the body range if you have already detected the body-start; otherwise omit the range and harvest the whole PDF (a cover image or other front-matter crop is harmless and is dropped by the Finalize sweep if no report uses it). It crops the genuine figures — diagrams, plots, photos — into `ocr/figures/fig-pNNN-K.ext` (named by absolute PDF page) and writes `ocr/figures.md` (Label / File / Page / Caption). Tables and console output are deliberately **not** cropped: the text extraction already carries them. MinerU can take minutes and downloads models on first run — use a generous timeout. On `mineru` missing (exit 3) relay the install hint and continue; on any other failure, report it and continue without figures. The crops are consumed by Phase 1 (assigned to chunks) and swept for coverage at Finalize. This phase writes only files — no page content enters the orchestrator's context.
...
- [ ] If `mineru` was available, `ocr/figures.md` exists and **every file in `ocr/figures/` is referenced-and-explained by some report** (no orphan crop); its "⚠ Not extracted" rows, if any, were worked off. If `mineru` was absent, the manifest says figure harvest was skipped.
