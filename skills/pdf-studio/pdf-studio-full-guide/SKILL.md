...
description: This skill should be used when the user wants to run the WHOLE pdf-studio pipeline end-to-end on one PDF in a single request — from summary, through a detailed report for every chapter, to a two-host dialogue script, to synthesized audio. Triggers on "この本を全部やって", "一冊まるごと音声ガイドまで", "summaryから音声まで一気に", "全ステップ実行して", "run the whole pipeline / do everything for this PDF / from summary to audio". It is the orchestrator that chains pdf-studio-summarize → pdf-studio-deep-dive (per chapter) → pdf-studio-audio-dialogue → pdf-studio-audio-narrate, confirming scope once up front and then running the phases. Should NOT trigger for a single phase (use pdf-studio-summarize, pdf-studio-deep-dive, pdf-studio-audio-dialogue, or pdf-studio-audio-narrate directly), or for a PDF already partway through the pipeline where only the remaining steps are wanted (invoke the remaining sub-skills).
version: 0.1.0
user-invocable: true
---
