...
description: This skill should be used when the user wants pdf-studio reports turned into a real website — authored web pages (not converted Markdown) browsable from a smartphone — building a static site under a work dir's site/ from its reports/ with the audio/ guides playable in-page. Triggers on "レポートをWebサイトにして", "HTMLにして", "サイトを作って", "スマホで読めるようにして", "generate a site from the reports", "make a website from the reports". Should NOT trigger for deploying/hosting the built site online (use pdf-studio-deploy-site), for producing the reports themselves (use pdf-studio-summarize / pdf-studio-deep-dive), or for creating the audio guide (use pdf-studio-audio-dialogue / pdf-studio-audio-narrate).
version: 0.4.0
user-invocable: true
---
...
The pages are **authored, not converted**: the `pdf-studio-site-page` procedure rewrites each report for web reading (lede, key-points box, scannable sections) against a bundled design system. Rendering the Markdown 1:1 into HTML is explicitly NOT the deliverable. The output is a **self-contained static site** (no server-side, no CDN/external references) — but it does ship a bundled `assets/app.js` that adds client-side interactivity (reading-progress bar, table of contents with scroll-spy, light/dark theme toggle, back-to-top, index filter) as progressive enhancement: every page stays fully readable if JS is disabled.

The shared stylesheet `style.css` is owned by the **`pdf-studio-site-base`** skill (the library index uses the same design system); this skill's own `assets/` holds `app.js`, `page.html`, and `index.html`. If `pdf-studio-site-base` is not installed, stop and say so rather than guessing a stylesheet path.
...
2. Create `<WORK_DIR>/site/assets/` and `site/audio/`; copy BOTH the shared stylesheet `pdf-studio-site-base/assets/style.css` AND this skill's `pdf-studio-generate-site/assets/app.js` into `site/assets/`, and the audio files into `site/audio/`. `site/` is disposable — regenerating overwrites it; never hand-edit it (edit `reports/` and regenerate instead).
...
- [ ] The site is self-contained: `grep -o 'src="[^"]*"\|href="[^"]*"' site/*.html` shows only relative paths (plus links the reports themselves contained). Both `site/assets/style.css` and `site/assets/app.js` exist, and every page's `<head>` keeps the theme-boot inline script and the `assets/app.js` reference.
