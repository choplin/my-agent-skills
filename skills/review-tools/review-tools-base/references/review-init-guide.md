...
ingestion op (or `review-tools-resolve` invoked directly) creates it; later ops
append to it.
...
2. **Standalone default** — otherwise `.agents/review-tools/{name}/`, where `{name}` =
...
2. Read `review-tools-base` skill (`references/review-template.md`).
...
of truth (see `review-tools-base` skill (`references/review-state.md`)). Source ledgers
