...
- Or use `/dev-workflow-resume-work`
...
If EnterPlanMode is used during task implementation, add a `## dev-workflow Context` block to the plan file. Use the template in `dev-workflow-base` skill (`references/plan-mode-context.md`) with these values:
