...
- **Reference kinds.** A `references/` file is knowledge to consult (*read X when …*), a procedure to run (*apply X to …*), or a contract to conform to (*conform to X*); `SKILL.md` declares which with that verb. Split the kinds into `references/{knowledge,procedures,contracts}/` only when a skill holds more than one; a single-kind skill stays flat. Shape-only material (templates, verbatim text, images) goes in `assets/`. Details: the `skill-quality-standard` skill.
