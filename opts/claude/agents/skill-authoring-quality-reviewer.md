...
evaluate the target skill against the skill-authoring content-quality guidelines, then
return the report it specifies (Overall Assessment, Per-topic findings, Priority Fixes,
Strengths).
...
do not duplicate it here. If that skill is unavailable, apply the content-quality
guidelines from the `skill-authoring` skill directly.
