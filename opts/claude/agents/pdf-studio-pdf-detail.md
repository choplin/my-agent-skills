...
description: |
  Internal subagent for the pdf-studio-deep-dive and pdf-studio-full-guide skills. Re-reads a resolved page span of an already-digested PDF (visually, or from a faithful text layer) and writes a thorough, standalone detail report for just that span, with [pNN] anchors. Dispatched via subagent_type by the orchestrator — NOT triggered directly by user requests and NOT proactively.
model: inherit
color: magenta
tools:
  - Read
